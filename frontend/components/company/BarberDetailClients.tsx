  ...
  return (
    <div className="bg-[#FAFAF6] min-h-screen font-sans">
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-10">
        <div className="flex flex-col md:flex-row gap-10 items-start">
          <div className="relative flex-shrink-0 w-52 h-52">
            ...
            <img src={`/${barberSlug}.png`} alt={barberName} className="absolute inset-0 w-full h-full object-cover object-top rounded-full border-4 border-white shadow-md" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          </div>
          <div className="flex-1">
            <span className="inline-block border-2 border-[#1a1a1a] text-[#1a1a1a] text-[11px] font-bold px-3 py-1 rounded-full tracking-widest uppercase mb-3">{data.title}</span>
            <h1 className="text-[40px] md:text-[52px] font-black leading-none tracking-tight text-[#1a1a1a] mb-1">{barberName}</h1>
            <p className="text-[#7B5EA7] font-bold text-lg mb-3">{self?.role}</p>
            <div className="flex items-center gap-2 mb-3">
              <div className="flex gap-0.5">{[1,2,3,4,5].map((i) => <svg key={i} className="w-5 h-5 text-[#F9C74F]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>)}</div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center gap-3 mb-6"><span className="text-[#178E81] text-[11px] font-extrabold tracking-[3px] uppercase">Portfolio</span><div className="flex-1 h-px bg-gray-200" /></div>
        <h2 className="text-3xl font-black text-[#1a1a1a] mb-6"><span className="text-[#7B5EA7]">{firstName}'s</span> Best Work</h2>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {data.portfolio.map((img, i) => (
            <div key={i} className="rounded-2xl overflow-hidden aspect-square bg-gray-200 hover:scale-105 transition-transform"><img src={img} alt={`Porto ${i + 1}`} className="w-full h-full object-cover" /></div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-6">
        <div className="flex flex-col md:flex-row gap-5 items-stretch">
          <div className="flex-1 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <ul className="space-y-3">{data.speciality.map((s, i) => <li key={i} className="flex items-center gap-3 text-sm font-medium text-[#1a1a1a]"><span className="w-5 h-5 bg-[#F9C74F] rounded-full flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></span>{s}</li>)}</ul>
          </div>
          <div className="flex-1 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <p className="text-[#178E81] text-[11px] font-extrabold tracking-[3px] mb-4 uppercase">Signature Tools</p>
            <ul className="space-y-3">{data.tools.map((t, i) => <li key={i} className="flex items-center gap-3 text-sm font-medium text-[#1a1a1a]"><span className="w-5 h-5 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-[#1a1a1a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.121 14.121L19 19m-7-7l7-7m-7 7l-2.879 2.879M12 12L9.121 9.121m0 5.758a3 3 0 10-4.243 4.243 3 3 0 004.243-4.243zm0-5.758a3 3 0 10-4.243-4.243 3 3 0 004.243 4.243z"/></svg></span>{t}</li>)}</ul>
          </div>
          <div className="flex flex-col items-center justify-center gap-4 bg-[#1a1a1a] rounded-2xl px-6 py-8 min-w-[200px] text-center">
            <svg className="w-10 h-10 text-[#F9C74F]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            <p className="text-white text-sm font-bold leading-snug">Mau Dapetin Cukuran Keren dari <span className="text-[#F9C74F]">{firstName}</span>?</p>
            <Link href="/booking" className="bg-[#F9C74F] hover:bg-yellow-400 text-[#1a1a1a] text-sm font-extrabold px-5 py-2.5 rounded-xl transition-colors whitespace-nowrap">Book Now →</Link>
          </div>
        </div>
      </section>
